<template>
		<view class="scene" :style="{color:colortext,fontSize:fontSize}">
			<!--  @touchstart.stop.prevent="touchstartscene" @touchend.stop.prevent="touchendscene" @touchmove.stop.prevent="touchmovescene" -->
			<view class="box" :style="oDiv">
				<view class="ring">
					<view class="ringtext" :style="{transform:item.style}" v-for="(item,index) in liviews" :key="index">{{item.text}}</view>
				</view>
			</view>
		</view>
	</view>
...
<script>
	export default {
		props: {
			automaticRotate: {
				type: Number,
				default:1 //自动滚动速度，越高滚动越慢,0不滚动，设置负数，则反方向滚动
			},
			manualRotate:{
				type: Number,
		...
		created() {
		     ...
		     var columnNum = 0;
		         ...
		         for(var i=0; i<this.liview.length; i++){
		             ...
		             this.setCss3(this.liview[i],{transform:'translate3D('+ this.liview[i].circleX +'px,'+ this.liview[i].circleY +'px,'+ this.liview[i].circleZ +'px) rotateY('+ this.liview[i].circlePhi +'rad) rotateX('+ (this.liview[i].circleTheta-Math.PI/2) +'rad)'},this);
